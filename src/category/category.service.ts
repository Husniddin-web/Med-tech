...
export class CategoryService {
	...
	async create(dto: CreateCategoryDto) {
		...
		const invalidLanguageIds = languageIds!.filter((id) => !existingLanguageIds.has(id));
			...
			data: {
				...
				translations: {
					upsert: dto.translations?.map((t) => ({
						where: {
							categoryId_languageId: { categoryId: id, languageId: t.languageId },
						},
						update: { name: t.name },
						create: { name: t.name, languageId: t.languageId },
					})),
				},
			},
			include: { translations: true },
	...
	async addCategoryTranslation(id: number, dto: CreateCategoryTranslationDto) {
		...
		const isLanguage = await this.prisma.language.findUnique({ where: { id: dto.languageId } });

		if (!isLanguage) {
			throw new BadRequestException(`Language is not found with id #${dto.languageId}`);
		}
