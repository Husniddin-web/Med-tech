...
export class CategoryController {
  ...
  async create(
    ...
    createCategoryDto.logo = await this.fileUploadService.saveFile(logo);
    if (typeof createCategoryDto.translations == "string") {
      createCategoryDto.translations = JSON.parse(
  ...
  @ApiResponse({
    status: 200,
    description: "Successfully fetched categories by language",
  })
  findAllCategoryByLang(@CurrentLanguage() languageId: number) {
    return this.categoryService.findAllByLang(languageId);
  }
  ...
  @ApiResponse({
    status: 200,
    description: "Successfully fetched category by language and ID",
  })
  findOneByLang(
    @Param("id") id: number,
    @CurrentLanguage() languageId: number
  ) {
    return this.categoryService.findOneByLang(id, languageId);
  }
  ...
  updateTranslation(
    ...
    return this.categoryService.updateCategoryTranslation(
      +id,
      +languageId,
      dto
    );
  ...
  @ApiResponse({
    status: 201,
    description: "Category translation added successfully",
  })
  addTranslation(
    @Param("id") id: string,
